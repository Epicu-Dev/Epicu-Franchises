...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    if (req.method === 'GET') {
      ...
      // Filtre onglet = "Liens importants"
      const formulaParts: string[] = [`{Onglet} = "Liens importants"`];
    ...
    if (req.method === 'POST') {
      try {
        ...
        // Forcer l'onglet sur Liens importants
        fieldsToCreate['Onglet'] = 'Liens importants';
        if (commentaires) fieldsToCreate['Commentaires'] = commentaires;
        if (lien) fieldsToCreate['Lien'] = lien;

        // Date d'ajout : Airtable peut le gérer via champ date automatique; si fourni, on l'utilise
        ...
        console.error('link-importants POST error', err);
    ...
    if (req.method === 'PATCH') {
      try {
        ...
        // Toujours forcer l'onglet sur Liens importants si present ou non
        fieldsToUpdate['Onglet'] = 'Liens importants';

        // Date d'ajout peut être écrasée si présent explicitement
        ...
        console.error('link-importants PATCH error', err);
    ...
    console.error('Airtable error (link-importants):', error?.message || error);
