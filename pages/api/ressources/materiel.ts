...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    if (req.method === 'GET') {
      ...
      // Filtre onglet = "Matériel"
      const formulaParts: string[] = [`{Onglet} = "Matériel"`];
    ...
    if (req.method === 'POST') {
      try {
        ...
        // Forcer l'onglet sur Matériel
        fieldsToCreate['Onglet'] = 'Matériel';
        ...
        console.error('materiel POST error', err);
    ...
    if (req.method === 'PATCH') {
      try {
        ...
        // Toujours forcer l'onglet sur Matériel
        fieldsToUpdate['Onglet'] = 'Matériel';
        ...
        console.error('materiel PATCH error', err);
    ...
    console.error('Airtable error (materiel):', error?.message || error);
